...
struct LogIcon: View {
    ...
    var body: some View {
        ...
        ZStack {
            Capsule()
                .fill(tint)
                .overlay(Capsule().stroke(SylvanTheme.darkWood.opacity(0.5), lineWidth: 1.2))
                .frame(width: 36, height: 17)
            Ellipse()
                .fill(SylvanTheme.parchmentLight)
                .overlay(Ellipse().stroke(SylvanTheme.darkWood.opacity(0.5), lineWidth: 1))
