        ...
        TimelineView(.periodic(from: .now, by: 1)) { context in
            ...
            HStack(spacing: 6) {
                Image(systemName: "play.rectangle.fill")
                    ...
                    .foregroundStyle(TimberlineTheme.hudTextDark.opacity(0.4))
                Text(formattedCountdown(remaining))
                    .font(.stat(11, weight: .semibold))
                    .foregroundStyle(TimberlineTheme.hudTextDark.opacity(0.6))
                    .monospacedDigit()
            }
            ...
            .background(
                Capsule()
                    .fill(.ultraThinMaterial)
                    .background(Capsule().fill(TimberlineTheme.hudPanelTint.opacity(0.7)))
            )
        }
    }
