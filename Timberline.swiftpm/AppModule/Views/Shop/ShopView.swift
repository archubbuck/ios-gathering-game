...
struct AxeRowView: View {
                    ...
                    Text("Requires level \(axe.levelReq)")
                        ...
                        .foregroundStyle(
                            game.level >= axe.levelReq
                                ? TimberlineTheme.textOnParchmentSecondary
                                : TimberlineTheme.destructive
                        )
                }
            }
    ...
    private var actionButton: some View {
        ...
        case .buyable, .tooPoor:
            ...
            .buttonStyle(.borderedProminent)
            .tint(state == .tooPoor ? TimberlineTheme.locked : TimberlineTheme.gold)
            .font(.stat(13))
            .disabled(state == .tooPoor)
        case .levelLocked:
            Label("Lv \(axe.levelReq)", systemImage: "lock.fill")
                .font(.stat(13))
                .foregroundStyle(TimberlineTheme.locked)
        }
    }
}
