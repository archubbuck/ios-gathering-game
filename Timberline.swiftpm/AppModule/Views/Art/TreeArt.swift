...
private struct Trunk: View {
    ...
    var color: Color = TimberlineTheme.bark

    var body: some View {
        TrunkShape()
            .fill(color)
            .overlay(TrunkShape().stroke(TimberlineTheme.darkWood.opacity(0.6), lineWidth: 1.5))
...
private struct Canopy: View {
    ...
    var body: some View {
        ...
        .frame(width: width, height: height)
        .overlay(Ellipse().stroke(TimberlineTheme.darkWood.opacity(0.4), lineWidth: 1.5))
    }
}
...
struct StumpArt: View {
    var body: some View {
        VStack(spacing: 0) {
            Ellipse()
                .fill(TimberlineTheme.parchmentEdge)
                .overlay(Ellipse().stroke(TimberlineTheme.darkWood.opacity(0.6), lineWidth: 1.5))
                .frame(width: 26, height: 10)
                .zIndex(1)
            TrunkShape()
                .fill(TimberlineTheme.bark)
                .overlay(TrunkShape().stroke(TimberlineTheme.darkWood.opacity(0.6), lineWidth: 1.5))
...
private struct BirchArt: View {
    var body: some View {
        ZStack {
                ...
                .overlay(
                    VStack(spacing: 9) {
                        ForEach(0..<4, id: \.self) { index in
                            Capsule()
                                .fill(TimberlineTheme.darkWood.opacity(0.55))
            ...
            Canopy(
                width: 52, height: 48,
                base: TimberlineTheme.canopyLight,
    ...
    var body: some View {
        ZStack {
            Trunk(width: 16, height: 46)
                .offset(y: 36)
            Canopy(width: 46, height: 42, base: TimberlineTheme.canopyDark, highlight: TimberlineTheme.forestGreen)
                .offset(x: -22, y: -2)
            Canopy(width: 46, height: 42, base: TimberlineTheme.canopyDark, highlight: TimberlineTheme.forestGreen)
                .offset(x: 22, y: -2)
            Canopy(
                width: 56, height: 50,
                base: TimberlineTheme.forestGreen,
                highlight: TimberlineTheme.canopyLight
            )
            .offset(y: -24)
        }
    }
    ...
    var body: some View {
        ZStack {
            ...
            ForEach(0..<3, id: \.self) { layer in
                TriangleShape()
                    .fill(layer.isMultiple(of: 2) ? TimberlineTheme.canopyDark : TimberlineTheme.forestGreen)
                    .overlay(TriangleShape().stroke(TimberlineTheme.darkWood.opacity(0.4), lineWidth: 1.5))
                    .frame(
