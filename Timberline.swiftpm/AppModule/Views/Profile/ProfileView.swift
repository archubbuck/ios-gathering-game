    ...
    private func statRow(symbol: String, label: String, value: String) -> some View {
        HStack {
            Image(systemName: symbol)
                .font(.system(size: 13))
                .foregroundStyle(TimberlineTheme.forestGreen)
                .frame(width: 22)
            Text(label)
...
struct AchievementBadge: View {
    ...
    var body: some View {
        VStack(spacing: 4) {
            ZStack {
                Circle()
                    .fill(unlocked ? TimberlineTheme.heroGradient : LinearGradient(
                        colors: [TimberlineTheme.locked, TimberlineTheme.locked.opacity(0.7)],
                        startPoint: .top, endPoint: .bottom
                    ))
                Circle()
                    .stroke(unlocked ? TimberlineTheme.gold : TimberlineTheme.locked, lineWidth: 2)
            ...
            Text(achievement.name)
                .font(.stat(10))
                .foregroundStyle(
                    unlocked ? TimberlineTheme.textOnParchment : TimberlineTheme.textOnParchmentSecondary
                )
                .lineLimit(1)
        }
        .frame(width: 76)
